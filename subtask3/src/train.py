...
def train_svm_model(config: ConfigManager, train_data: pd.DataFrame, language: str, logger: logging.Logger) -> Dict:
    """Train SVM multi-label model."""
    
    logger.info("="*50)
    logger.info(f"Training SVM Multi-Label Model for {language.upper()}")
    logger.info("="*50)
...
def main(args):
    ...
    logger.info("Starting Subtask 3: Manifestation Identification")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train models for Subtask 3: Manifestation Identification")
